...
from app.manage.converters import role_out
from app.manage.deps import get_current_manage_user
from app.manage.models import Permission, Role, User
from app.manage.schemas import RoleCreate, RoleOut, RoleUpdate
...
def create_role(
    ...
    r = Role(
        ...
        permissions=perms,
        role_type=RESOURCE_TYPE_CUSTOM,
    )
    db.add(r)
    db.commit()
    db.refresh(r)
...
def update_role(
    ...
    if r is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="角色不存在")
    if r.role_type == RESOURCE_TYPE_BUILTIN and body.name not in (None, r.name):
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="内置角色不可改名")
...
def delete_role(
    ...
    if r is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="角色不存在")
    if r.role_type == RESOURCE_TYPE_BUILTIN:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="不能删除内置角色")
    db.delete(r)
    db.commit()
